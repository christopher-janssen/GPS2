...
# Import GPS data with batch processing and error handling
...
tryCatch({
  ...
  message("Reading GPS data file...")

  # Read GPS data with explicit column types
  ...
  message("✓ Read ", nrow(gps_data), " rows from file")
  ...
  message("✓ Filtered to ", nrow(gps_data), " rows with valid data")

  if (nrow(gps_data) == 0) {
    stop("No valid GPS data after filtering!")
  }
  ...
  if (nrow(subject_lookup) == 0) {
    stop("No subjects found in database! Run subject import first.")
  }

  gps_data <- gps_data |>
    left_join(subject_lookup, by = "subid") |>
    filter(!is.na(id)) |>  # Drop GPS points with no matching subject
    select(-subid) |>
    rename(subject_id = id)

  message("✓ Matched ", nrow(gps_data), " GPS points to ",
          length(unique(gps_data$subject_id)), " subjects")

  # Insert data in chunks
  n_rows <- nrow(gps_data)
  n_chunks <- ceiling(n_rows / chunk_size)

  message("Inserting ", n_rows, " rows in ", n_chunks, " chunks...")
  ...
  # Verify import
  db_count <- dbGetQuery(con, "SELECT COUNT(*) as n FROM raw_gps_points")$n
  message("✓ Completed GPS data import - ", total_imported, " points inserted")
  message("✓ Database contains ", db_count, " GPS points")

  if (db_count != total_imported) {
    warning("Mismatch: inserted ", total_imported, " but database has ", db_count)
  }

  beepr::beep()

}, error = function(e) {
  message("✗ GPS import failed with error:")
  message(e$message)
  beepr::beep(sound = 2)  # Error beep
  stop("GPS import failed. See error above.")
})
```
