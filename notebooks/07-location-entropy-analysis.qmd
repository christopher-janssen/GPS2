...
**Key Insight**: Each GPS point in `processed_gps_points` now has a `cluster_id` linking it to a specific location cluster from `gps_clusters`. This enables us to:
...
```{r store-results}
# Create table for daily location entropy
dbExecute(con, "DROP TABLE IF EXISTS daily_location_entropy CASCADE")
...
dbExecute(con, "
  CREATE INDEX IF NOT EXISTS idx_daily_entropy_subject
  ON daily_location_entropy(subject_id)
")

dbExecute(con, "
  CREATE INDEX IF NOT EXISTS idx_daily_entropy_date
  ON daily_location_entropy(date)
")

cat("✓ Indexes created for efficient querying\n")
```
