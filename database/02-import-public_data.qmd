...
landuse_path <- str_c(path_gps_public, "/osm_landuse_wisconsin.gpkg")
...
osm_landuse |>
  slice_head(n = 10) |>
  st_drop_geometry() |>
  select(osm_id, class, type, name, city, area_sqkm)
```
...
osm_landuse |>
  ...
  select(osm_id, class, type, name, area_sqkm) |>
  slice_head(n = 10)
```
...
This section loads Area Deprivation Index (ADI) scores by census block group for Wisconsin.

**Source:** `adi_2020.gpkg` (2020 census block groups)
**Target:** `public_data.adi_scores`
**Expected:** ~20k block groups (Wisconsin)

## Load ADI from GeoPackage

```{r}
# Path to ADI data
adi_path <- str_c(path_shared, "/adi/adi_2020.gpkg")

# Read ADI data
adi_data <- st_read(adi_path)
```
...
adi_data |>
  ...
  summarise(
    ...
    missing = sum(is.na(adi_national_percentile))
  )
```
...
SELECT
    ...
    COUNT(*) FILTER (WHERE adi_national_percentile IS NULL) as missing
