...
# Remove water and calculate land area fraction
grid_madison <- grid_madison |>
  mutate(
    cell_area = st_area(st_geometry(.)),
    water_area = st_area(st_intersection(st_geometry(.), st_union(water))),
    water_area = ifelse(is.na(water_area), units::set_units(0, "m^2"), water_area),
    land_area = cell_area - water_area,
    land_fraction = as.numeric(land_area / cell_area)
  ) |>
  filter(land_fraction > 0)  # Remove water-only cells
