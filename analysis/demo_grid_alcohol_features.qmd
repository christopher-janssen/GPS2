...
For exploring spatial patterns in detail, use this interactive map.
...
# Create interactive map with density overlay
tm_shape(grid_enriched) +
  tm_fill(
    "n_1000ft",
    palette = "YlOrRd",
    style = "pretty",
    title = "Outlets within 1000ft",
    alpha = 0.7,
    popup.vars = c(
      "Cell ID" = "cell_id",
      "500ft" = "n_500ft",
      "1000ft" = "n_1000ft",
      "1500ft" = "n_1500ft",
      "Nearest (m)" = "dist_nearest_m"
    )
  ) +
  tm_borders(col = "white", lwd = 0.5)

# Reset to plot mode for remaining visualizations
tmap_mode("plot")
```
...
p_offpremise <- tm_shape(grid_enriched) +
  ...
  tm_borders(alpha = 0.3, lwd = 0.8) +
  tm_layout(
    title = "Liquor Stores, Wine Shops",
    title.size = 1.5,
    legend.text.size = 1.2,
    legend.title.size = 1.4
  )
...
saveRDS(
  grid_enriched,
  here(path_terrain, "grid_madison_alcohol_features.rds")
)

# Also save as GeoPackage for QGIS visualization
st_write(
  grid_enriched,
  here(path_terrain, "grid_madison_alcohol_features.gpkg"),
  delete_dsn = TRUE
)
```
