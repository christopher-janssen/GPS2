...
```{r}
# Get sample of 10,000 GPS points from raw_gps
...
ORDER BY g.point_id, distance_m;
")
...
subject_poi_matches AS (
  SELECT
    ...
    COUNT(DISTINCT CASE
      ...
      THEN g.point_id END) as poi_match_count
  FROM risk1.raw_gps g
...
subject_alcohol_exposure AS (
  SELECT
    ...
    COUNT(DISTINCT CASE
      ...
      THEN g.point_id END) as alcohol_venue_matches
  FROM risk1.raw_gps g
  LEFT JOIN public_data.osm_poi p ON 1=1
  GROUP BY g.subid
)
...
subject_profiles |>
  knitr::kable(
    col.names = c("Subject ID", "GPS Points", "POI Matches", "Match Rate %", "Alcohol Venues"),
    format.args = list(big.mark = ",")
  )
```
...
selected_subjects |>
  ...
  knitr::kable(
    col.names = c("Profile", "Subject ID", "GPS Points", "POI Matches", "Match Rate %", "Alcohol Venues"),
    format.args = list(big.mark = ",")
  )
```
...
if (nrow(subj1_data) > 0) {
  subj1_data |>
    arrange(time) |>
    slice_head(n = 15) |>
    select(time, nearest_poi_class, nearest_poi_type, distance_m) |>
    knitr::kable(
      col.names = c("Date/Time", "POI Class", "POI Type", "Distance (m)"),
      digits = 2
    )
}
```
...
comparison <- selected_subjects |>
  ...
  select(profile, subid, n_points, poi_matches, poi_match_rate_pct, alcohol_venue_matches)
