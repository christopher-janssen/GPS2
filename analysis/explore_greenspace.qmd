...
Pull sample of GPS points for enrichment.

```{r}
# Get 10,000 GPS points from raw_gps
...
gps_sample |>
  ...
  knitr::kable(
    col.names = c("Subjects", "GPS Points"),
    digits = 1
  )
```
...
## GPS Points in Parks/Nature

Count GPS points in greenspace vs. built environment (note: without dwell time data, we count points rather than time spent).

```{r}
# Aggregate points by greenspace category
points_by_greenspace <- gps_greenspace |>
...
points_by_greenspace |>
  knitr::kable(
    col.names = c("Environment Type", "GPS Points", "% of Points"),
    format.args = list(big.mark = ","),
    digits = 1
  )
```
...
greenspace_summary |>
  knitr::kable(
    col.names = c("Environment", "GPS Points", "% of Points"),
    format.args = list(big.mark = ","),
    digits = 1
  )
```
...
```{r}
# Calculate greenspace exposure per subject (by point counts)
subject_greenspace <- gps_greenspace |>
  ...
  mutate(
    pct_points_greenspace = round(100 * greenspace_points / total_points, 1)
  ) |>
  arrange(desc(pct_points_greenspace))

# Summary statistics
subject_greenspace |>
  summarise(
    n_subjects = n(),
    avg_pct_greenspace = mean(pct_points_greenspace, na.rm = TRUE),
    median_pct_greenspace = median(pct_points_greenspace, na.rm = TRUE),
    min_pct = min(pct_points_greenspace, na.rm = TRUE),
    max_pct = max(pct_points_greenspace, na.rm = TRUE)
  ) |>
  knitr::kable(
    col.names = c("Subjects", "Mean % Points", "Median % Points", "Min %", "Max %"),
    digits = 1
  )
```
...
subject_greenspace |>
  slice_head(n = 10) |>
  knitr::kable(
    col.names = c("Subject", "Total Points", "Greenspace Points", "% Points in Greenspace"),
    digits = 1
  )
```
...
gps_park_access <- gps_greenspace |>
  left_join(park_distance, by = "point_id") |>
  mutate(
...
subject_greenspace_all <- gps_greenspace |>
  ...
  mutate(
    pct_points_greenspace = round(100 * greenspace_points / total_points, 1),
    pct_points_parks = round(100 * park_points / total_points, 1)
  ) |>
  arrange(desc(pct_points_greenspace))
...
subject_greenspace_all |>
  slice_head(n = 20) |>
  knitr::kable(
    col.names = c("Subject ID", "GPS Points", "Greenspace Points", "Park Points", "% Points Greenspace", "% Points Parks"),
    digits = 1,
    format.args = list(big.mark = ",")
  )
```
...
if (nrow(subject_greenspace_all) > 0) {
  ...
  moderate_gs <- subject_greenspace_all |>
    filter(pct_points_greenspace > 5, pct_points_greenspace < 15) |>
...
if (nrow(selected_greenspace_subjects) > 0) {
  selected_greenspace_subjects |>
    knitr::kable(
      col.names = c("Profile", "Subject ID", "GPS Points", "Greenspace Points", "Park Points", "% Points Greenspace", "% Points Parks"),
      digits = 1,
      format.args = list(big.mark = ",")
    )
} else {
...
if (nrow(selected_greenspace_subjects) >= 1) {
  ...
  cat(sprintf("\n**Subject %d:** %.1f%% of GPS points in greenspace (%d of %d points)\n\n",
              subj1_gs_id,
              selected_greenspace_subjects$pct_points_greenspace[1],
              selected_greenspace_subjects$greenspace_points[1],
              selected_greenspace_subjects$total_points[1]))
  ...
  subj1_gs_breakdown <- subj1_gs_data |>
    ...
    mutate(
      pct_of_greenspace_points = round(100 * n_points / sum(n_points), 1)
    ) |>
    arrange(desc(n_points))

  subj1_gs_breakdown |>
    knitr::kable(
      col.names = c("Greenspace Type", "GPS Points", "% of Greenspace Points"),
      digits = 1,
      format.args = list(big.mark = ",")
    )
  ...
  if (nrow(subj1_gs_data |> filter(is_greenspace)) > 0) {
    subj1_gs_data |>
      filter(is_greenspace) |>
      select(point_id, greenspace_category, landuse_type, landuse_area_sqkm) |>
      slice_head(n = 15) |>
      knitr::kable(
        col.names = c("GPS Point ID", "Greenspace Type", "Landuse Type", "Area (km²)"),
        digits = 2
      )
  }
} else {
...
if (nrow(selected_greenspace_subjects) >= 2) {
  ...
  cat(sprintf("\n**Subject %d:** %.1f%% of GPS points in greenspace (%d of %d points)\n\n",
              subj2_gs_id,
              selected_greenspace_subjects$pct_points_greenspace[2],
              selected_greenspace_subjects$greenspace_points[2],
              selected_greenspace_subjects$total_points[2]))

  # Where are GPS points instead?
  subj2_env_breakdown <- subj2_gs_data |>
    ...
    mutate(
      pct_of_points = round(100 * n_points / sum(n_points), 1)
    ) |>
    arrange(desc(n_points))
  ...
  cat("Subject 2 data not available.\n")
}
```
...
if (nrow(selected_greenspace_subjects) >= 3) {
  ...
  cat(sprintf("\n**Subject %d:** %.1f%% of GPS points in greenspace (%d of %d points)\n\n",
              subj3_gs_id,
              selected_greenspace_subjects$pct_points_greenspace[3],
              selected_greenspace_subjects$greenspace_points[3],
              selected_greenspace_subjects$total_points[3]))
...
if (nrow(selected_greenspace_subjects) > 0) {
  ...
  greenspace_comparison <- selected_greenspace_subjects |>
    select(profile, subid, total_points, greenspace_points, pct_points_greenspace, park_points, pct_points_parks)

  greenspace_comparison |>
    knitr::kable(
      col.names = c("Profile", "Subject ID", "Total Points", "Greenspace Points", "% Greenspace", "Park Points", "% Parks"),
      digits = 1,
      format.args = list(big.mark = ",")
    )
} else {
