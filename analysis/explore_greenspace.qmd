...
```{r}
# Join with original GPS data to exclude points already in parks
gps_park_access <- gps_landuse |>
...
selected_greenspace_subjects <- bind_rows(
  subject_greenspace_all |>
    arrange(desc(pct_time_greenspace)) |>
    slice(1) |>
  ...
  subject_greenspace_all |>
    ...
    slice(1) |>
    mutate(profile = "Moderate Greenspace")
) |>
