...
categories within 50m

**Distance Limit:** 50 meters (captures direct visits + immediate
nearby exposure)
...
**Distance threshold:** 50 meters captures direct visits where person is AT or very near the POI
...
``` r
# For each GPS point, find nearest POI of ANY type within 50m
gps_data |>
  group_by(row_id) |>
  mutate(
    nearest_poi_result = find_closest_poi(this_point, all_poi_categories, max_dist = 50)
  ) |>
  filter(distance <= 50) |>
  slice_head(n = 1)
```
...
-   `ST_DWithin(..., 50)`: Pre-filter to 50m radius
...
CROSS JOIN LATERAL (
    ...
    FROM public_data.osm_poi_visitable poi
    ORDER BY poi.geom <-> gps.geom
    LIMIT 5
) poi
WHERE ST_DWithin(poi.geom::geography, gps.geom::geography, 50)
...
gps_enriched |>
  summarise(
    total_points = n(),
    with_poi = sum(!is.na(pass_poi_category)),
    match_rate_pct = round(sum(!is.na(pass_poi_category)) / n() * 100, 2)
  ) |>
  knitr::kable()
```
...
```{r}
output_path <- str_c(path_shared, "/gps_enriched_poi.csv")
