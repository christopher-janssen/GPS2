...
-   **Nearest POI**: Hierarchical matching for environmental context and lapse prediction
    ...
    **Tier 2 - Moderate Evidence (Environmental Context):**
    ...
    - Parks/green spaces: protective factor, healthy coping environments
    - Healthcare facilities: treatment engagement, routine care
    - Grocery/convenience stores: routine activities, alcohol access points
...
**Purpose:** Hierarchical POI matching for environmental contextualization and lapse prediction.

**Approach:** For each GPS point, find the single nearest POI across all categories
(Tier 1 alcohol/recovery + Tier 2 environmental context). This captures **primary exposure** -
whichever environmental factor is closest.
...
- **Feature reduction flexibility:** Include/exclude categories during modeling phase
...
**Tier 2 - Moderate Evidence (Environmental Context):**
- **Restaurants/food** (restaurants, cafes, fast food) - social context, alcohol availability
- **Parks/green spaces** (parks, nature reserves) - protective, healthy coping
- **Healthcare facilities** (hospitals, clinics, pharmacies) - treatment engagement
- **Grocery/convenience** (supermarkets, convenience stores) - routine activities, alcohol access
...
**Note:** Feature selection happens during modeling. All categories enriched now,
then reduce to final feature set based on theory + sample size constraints (10-20 obs/feature).
...
**Feature selection strategy:** Start with all 7 categories during enrichment,
then apply theory-driven reduction during modeling to meet sample size constraints
(10-20 observations per feature with N=167 subjects).

**Example aggregation strategies:**

```{r eval=FALSE}
# Example 1: Proportion of time near each POI category (7 features)
subject_features <- gps_enriched |>
  ...
  summarize(
    ...
    # TIER 2: Environmental context
    pct_time_near_restaurant = mean(poi_category == "restaurant" & distance_m <= 50, na.rm = TRUE),
    pct_time_near_park = mean(poi_category == "park" & distance_m <= 100, na.rm = TRUE),
    pct_time_near_healthcare = mean(poi_category == "healthcare" & distance_m <= 100, na.rm = TRUE),
    pct_time_near_grocery = mean(poi_category == "grocery" & distance_m <= 50, na.rm = TRUE)
  )
...
# Example 4: Combined exposure metrics (reduced feature set)
subject_features <- gps_enriched |>
  group_by(subid) |>
  summarize(
    # Composite risky exposure (alcohol venues + retail + some restaurants)
    pct_time_risky = mean(poi_category %in% c("alcohol_venue", "alcohol_retail") & distance_m <= 100, na.rm = TRUE),

    # Composite protective exposure (recovery + parks + healthcare)
    pct_time_protective = mean(poi_category %in% c("recovery_facility", "park", "healthcare") & distance_m <= 100, na.rm = TRUE),

    # Risk/protective ratio
    risky_points = sum(poi_category %in% c("alcohol_venue", "alcohol_retail"), na.rm = TRUE),
    protective_points = sum(poi_category %in% c("recovery_facility", "park", "healthcare"), na.rm = TRUE),
    risk_protective_ratio = if_else(protective_points > 0, risky_points / protective_points, NA_real_),

    # Routine activity marker (grocery visits)
    n_routine_visits = sum(poi_category == "grocery" & distance_m <= 50, na.rm = TRUE)
  )

# Example 5: Theory-driven feature reduction (meets 10-20 obs/feature guideline)
# N=167 subjects → max 8-16 features at 20-10 obs/feature
subject_features <- gps_enriched |>
  group_by(subid) |>
  summarize(
    # Core alcohol exposure (MUST include - strongest theory)
    pct_alcohol_venue = mean(poi_category == "alcohol_venue" & distance_m <= 50, na.rm = TRUE),
    pct_alcohol_retail = mean(poi_category == "alcohol_retail" & distance_m <= 100, na.rm = TRUE),

    # Recovery engagement
    pct_recovery = mean(poi_category == "recovery_facility" & distance_m <= 100, na.rm = TRUE),

    # Protective activities (composite to reduce dimensionality)
    pct_healthy_environment = mean(poi_category %in% c("park", "healthcare") & distance_m <= 100, na.rm = TRUE),

    # Social/routine context (restaurants as alcohol-adjacent)
    pct_restaurant = mean(poi_category == "restaurant" & distance_m <= 50, na.rm = TRUE)
  )
# This creates 5 features → 167/5 = 33 obs/feature (well within guidelines)
```

**Sample size check:**

With N=167 subjects, multiple aggregation strategies are viable:

**All 7 categories separately:**
- 167 ÷ 7 = **24 observations per feature**
- Meets 10-20 obs/feature guideline
- Use if all categories show meaningful variance

**Reduced to 5 key features (Example 5 approach):**
- 167 ÷ 5 = **33 observations per feature**
- Conservative, well above minimum
- Recommended for initial modeling

**Composite risk/protective only (3 features):**
- 167 ÷ 3 = **56 observations per feature**
- Maximum power for small feature set
- Can support interaction terms and covariates

**Recommendation:** Enrich with all 7 categories now, decide on final feature set
during modeling based on variance and theoretical importance.
