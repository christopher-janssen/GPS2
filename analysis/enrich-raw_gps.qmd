...
This is basically the R equivalent of loading a dataset into the environment. We want it to be available and ready for analyses quickly, but not permanently.
...
- Polygon - enclosed area (cencus block groups/landuse)
- Multipoint, MultiPolygon, etc

2. 4326 = **SRID** (Spatial Reference System Identifier)

Specifies what coordinate system, and how to interpret the lat/lon numbers.
...
In Madison, Wisconsin's latitude (~43°)
- 1° lon = ~80km
- 1° lat = ~111km
...
PostgreSQL's GIST indexes require a single column. The geometry type packages both coordinates into one column, enabling spatial indexing. You cannot create a GIST spatial index on `(lat, lon)` as separate columns.

Use-case:

Queries will use both geometry and geography types:
- `geometry` when fast operations in degrees are necessary
- `geography` when accurate caluclations in meters are necessary
