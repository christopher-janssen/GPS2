...
SELECT
  CASE
    ...
    WHEN class = 'amenity' AND type IN ('restaurant', 'cafe')
      THEN 'restaurant'
    WHEN class = 'amenity' AND type IN ('fast_food', 'food_court')
      THEN 'fast_food'
...
**Brewery (now in Alcohol Venues):**

```{r}
dbGetQuery(con, "
SELECT class, type, COUNT(*) as count
FROM public_data.osm_poi
WHERE class = 'craft' AND type = 'brewery'
GROUP BY class, type
ORDER BY count DESC;
")
```
...
**Healthcare (merged with recovery clinics):**

```{r}
dbGetQuery(con, "
SELECT class, type, COUNT(*) as count
FROM public_data.osm_poi
WHERE (class = 'amenity' AND type IN ('hospital', 'clinic', 'doctors', 'pharmacy'))
   OR (class = 'healthcare' AND type IN ('clinic', 'centre'))
GROUP BY class, type
ORDER BY count DESC;
")
...
FROM public_data.osm_poi
WHERE class = 'amenity'
  AND type IN ('fast_food', 'food_court')
GROUP BY type
ORDER BY count DESC;
")
```
...
SELECT
    t.row_id,
    a.fips_2020 as pass_fips_2020,
    a.adi_national_percentile as pass_adi_national_percentile,
    a.adi_state_decile as pass_adi_state_decile
...
adi_results |>
  summarise(
    total_points = n(),
    with_adi = sum(!is.na(pass_adi_national_percentile)),
    match_rate = round(sum(!is.na(pass_adi_national_percentile)) / n() * 100, 2)
  )
```
...
SELECT DISTINCT ON (gps.row_id)
    gps.row_id,
    poi.class as pass_class,
    poi.type as pass_type,
    CASE
        -- TIER 1: Alcohol/Recovery (strongest evidence)
        WHEN poi.class = 'amenity' AND poi.type IN ('bar', 'pub', 'nightclub', 'biergarten')
            THEN 'alcohol_venue'
        WHEN poi.class = 'craft' AND poi.type = 'brewery'
            THEN 'alcohol_venue'
        ...
        WHEN (poi.class = 'amenity' AND poi.type IN ('hospital', 'clinic', 'doctors', 'pharmacy'))
            ...
            THEN 'healthcare'
        -- TIER 2: Environmental context (moderate evidence)
        WHEN poi.class = 'amenity' AND poi.type IN ('restaurant', 'cafe')
            THEN 'restaurant'
        WHEN poi.class = 'amenity' AND poi.type IN ('fast_food', 'food_court')
            THEN 'fast_food'
        ...
        WHEN poi.class = 'shop' AND poi.type IN ('convenience', 'supermarket', 'general', 'grocery')
            THEN 'grocery'
    END as pass_poi_category,
    ROUND(ST_Distance(gps.geom::geography, poi.geom::geography)::numeric, 2) as pass_distance_m
FROM temp_gps_points gps
CROSS JOIN LATERAL (
    SELECT class, type, geom
    FROM public_data.osm_poi poi
    WHERE
        -- TIER 1: Alcohol venues (on-premise + brewery)
        ...
        -- TIER 1: Social facilities (separate category)
        OR (poi.class = 'amenity' AND poi.type = 'social_facility')
        -- TIER 1: Healthcare (merged with recovery facilities)
        OR (poi.class = 'amenity' AND poi.type IN ('hospital', 'clinic', 'doctors', 'pharmacy'))
        OR (poi.class = 'healthcare' AND poi.type IN ('clinic', 'centre'))
        -- TIER 2: Restaurants (cafes only, no fast food)
        OR (poi.class = 'amenity' AND poi.type IN ('restaurant', 'cafe'))
        -- TIER 2: Fast food (separate category)
        OR (poi.class = 'amenity' AND poi.type IN ('fast_food', 'food_court'))
...
gps_enriched |>
  summarise(
    total_points = n(),
    with_adi = sum(!is.na(pass_adi_national_percentile)),
    match_rate_pct = round(sum(!is.na(pass_adi_national_percentile)) / n() * 100, 2)
  ) |>
  knitr::kable()
```
...
gps_enriched |>
  ...
  count(pass_poi_category) |>
  mutate(
    match_rate_pct = round(n / nrow(gps_enriched) * 100, 2),
    tier = case_when(
      pass_poi_category %in% c("alcohol_venue", "alcohol_retail", "social_facility", "healthcare") ~ "Tier 1",
      pass_poi_category %in% c("restaurant", "fast_food", "park", "grocery") ~ "Tier 2"
    ),
    category_label = case_when(
      pass_poi_category == "alcohol_venue" ~ "Alcohol Venues",
      pass_poi_category == "alcohol_retail" ~ "Alcohol Retail",
      pass_poi_category == "social_facility" ~ "Social Facilities",
      pass_poi_category == "healthcare" ~ "Healthcare",
      pass_poi_category == "restaurant" ~ "Restaurants",
      pass_poi_category == "fast_food" ~ "Fast Food",
      pass_poi_category == "park" ~ "Parks/Green Spaces",
      pass_poi_category == "grocery" ~ "Grocery/Convenience"
    ),
    exposure_type = case_when(
      pass_poi_category %in% c("alcohol_venue", "alcohol_retail") ~ "Risky",
      pass_poi_category %in% c("social_facility", "healthcare", "park") ~ "Protective",
      pass_poi_category %in% c("restaurant", "fast_food", "grocery") ~ "Mixed/Context"
    )
  ) |>
  arrange(tier, desc(n)) |>
...
gps_enriched |>
  ...
  summarise(
    total_matches = n(),
    mean_distance_m = round(mean(pass_distance_m, na.rm = TRUE), 1),
    median_distance_m = round(median(pass_distance_m, na.rm = TRUE), 1),
    min_distance_m = round(min(pass_distance_m, na.rm = TRUE), 1),
    max_distance_m = round(max(pass_distance_m, na.rm = TRUE), 1)
  ) |>
  knitr::kable()
```
