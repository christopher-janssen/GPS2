...
gt_no_intake <- filter_gt_by_proximity(gt_no_intake, stays_ref)


tibble(
  gt_set = c(
    "All locations (proximity-filtered)",
    "Intake only (proximity-filtered)",
    "No Intake (proximity-filtered)"
  ),
  n_locations = c(nrow(gt_all), nrow(gt_intake), nrow(gt_no_intake)),
  n_subjects = c(
    n_distinct(gt_all$subid),
...
comparison <- bind_rows(
  ...
  map_dfr(as.character(DWELL_THRESHOLDS),
          \(t) evaluate_threshold(t, gt_no_intake, "No Intake"))
)
