...
followup_dates <- visits_long |>
  filter(str_detect(visit_type, "followup") | visit_type == "end_study") |>
  group_by(subid) |>
  arrange(visit_date) |>
...
Side-by-side bar showing mean counts of concordant, DBSCAN-unique, and
...
overlap_details <- future_pmap_dfr(
  ...
  function(sub, fu_num, window_end,
    ...
    tp_locs <- gt_deduped[match$matched_b, ]
    ...
    tibble(
      ...
      n_tp_intake = sum(tp_locs$intake == 1),
      n_tp_algorithm = sum(tp_locs$intake == 0)
    )
  },
  stays_by_threshold = stays_by_threshold,
  locs_gt = locs_gt,
...
overlap_details |>
  ...
  summarize(
    ...
    concordance_intake = sum(n_tp_intake) /
      sum(n_tp_intake + n_fn_intake),
    concordance_algorithm = sum(n_tp_algorithm) /
      sum(n_tp_algorithm + n_fn_algorithm),
    .groups = "drop"
  ) |>
  knitr::kable(
    digits = 2,
    col.names = c("FU", "N", "Concordant (N)", "Legacy Only",
                  "DBSCAN Only", "Intake (N)",
                  "Algorithm (N)",
                  "Concordance % (Intake)",
